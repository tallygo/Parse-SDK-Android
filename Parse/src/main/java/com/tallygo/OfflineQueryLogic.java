...
/** package */ class OfflineQueryLogic {
  ...
  private static boolean matchesStatelessConstraint(String operator, Object constraint,
      Object value, ParseQuery.KeyConstraints allKeyConstraints) throws ParseException {
    switch (operator) {
      case "$ne":
  ...
  private <T extends ParseObject> ConstraintMatcher<T> createMatcher(ParseUser user,
      ...
      final ParseQuery.KeyConstraints allKeyConstraints) {
    switch (operator) {
      case "$inQuery":
  ...
  private <T extends ParseObject> ConstraintMatcher<T> createOrMatcher(ParseUser user,
      ArrayList<ParseQuery.QueryConstraints> queries) {
   ...
   */
  private <T extends ParseObject> ConstraintMatcher<T> createMatcher(ParseUser user,
      ParseQuery.QueryConstraints queryConstraints) {
    ...
    for (final String key : queryConstraints.keySet()) {
      ...
      if (key.equals("$or")) {
        ...
        ConstraintMatcher<T> matcher =
            createOrMatcher(user, (ArrayList<ParseQuery.QueryConstraints>) queryConstraintValue);
        matchers.add(matcher);
